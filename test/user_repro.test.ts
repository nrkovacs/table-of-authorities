...
import { parseText } from '../src/citation-parser';
import { CitationCategory } from '../src/citation-parser/types';

describe('User Reproduction', () => {
    const sampleText = `
...
`;
    ...
    test('should parse Brown v. Board of Education correctly', () => {
        const result = parseText(sampleText);
        const brown = result.citations.find(c => c.text.includes('347 U.S. 483'));

        expect(brown).toBeDefined();
        if (brown) {
            // This is the specific failure case: ensuring it captures just the name
            expect(brown.text).toBe('Brown v. Board of Education, 347 U.S. 483 (1954)');
            // Previous behavior captured: "As this Court held in Brown v. Board of Education..."
            expect(brown.text).not.toContain('As this Court held in');
        }
    });

    test('should parse Twombly correctly', () => {
        const result = parseText(sampleText);
        const twombly = result.citations.find(c => c.text.includes('Twombly'));

        expect(twombly).toBeDefined();
        if (twombly) {
            expect(twombly.text).toBe('Bell Atlantic Corp. v. Twombly, 550 U.S. 544 (2007)');
            // Previous behavior captured: "requires the court to accept..."
            expect(twombly.text).not.toContain('true. Bell');
        }
    });

    test('should parse Tribe treatise as Treatise, not Statute', () => {
        const result = parseText(sampleText);
        const tribe = result.citations.find(c => c.text.includes('Tribe'));

        expect(tribe).toBeDefined();
        if (tribe) {
            expect(tribe.category).toBe(CitationCategory.Treatises);
            expect(tribe.category).not.toBe(CitationCategory.Statutes);
        }
    });
    ...
    test('should parse Wright & Miller as Treatise, not Statute', () => {
        ...
        if (wm) {
            expect(wm.category).toBe(CitationCategory.Treatises);
            expect(wm.category).not.toBe(CitationCategory.Statutes);
        }
    });
});
